...
import { checkInteractions, checkInteractionsFromDB, drugSuggestions, type Medication, type AnalysisResult } from "@/services/interactionEngine";
...
export default function Analyzer() {
  ...
  const removeMedication = (i: number) => { setMedications((m) => m.filter((_, idx) => idx !== i)); setResult(null); };
  ...
  const selectSuggestion = (i: number, name: string) => { updateMedication(i, "name", name); setSearchQuery(null); };
  ...
  const analyze = async () => {
    ...
    if (user) {
      res = await checkInteractionsFromDB(valid);
      // Log usage
      ...
      <main className="flex-1 container py-10">
        <div className="max-w-3xl mx-auto">
            ...
            {medications.map((med, i) => (
              <div key={i} className="relative flex gap-3 items-start">
                <div className="flex-1 relative">
                  <Input placeholder="Drug name (e.g. Warfarin)" value={med.name} onChange={(e) => updateMedication(i, "name", e.target.value)} onFocus={() => setSearchQuery({ index: i, query: med.name })} onBlur={() => setTimeout(() => setSearchQuery(null), 200)} className="bg-card" />
                  {searchQuery?.index === i && suggestions.length > 0 && (
                    <div className="absolute z-10 top-full left-0 right-0 mt-1 bg-card border border-border rounded-lg shadow-elevated overflow-hidden">
                      {suggestions.map((s) => (
                        <button key={s} className="w-full text-left px-3 py-2 text-sm hover:bg-muted transition-colors" onMouseDown={() => selectSuggestion(i, s)}>{s}</button>
                      ))}
                    </div>
                  )}
                </div>
                <Input placeholder="Dosage" value={med.dosage} onChange={(e) => updateMedication(i, "dosage", e.target.value)} className="w-28 bg-card" />
                {medications.length > 1 && (
                  <button onClick={() => removeMedication(i)} className="mt-2 text-muted-foreground hover:text-destructive transition-colors"><X className="h-4 w-4" /></button>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="flex gap-3 mb-10">
            <Button variant="outline" onClick={addMedication} className="gap-2"><Plus className="h-4 w-4" /> Add Medication</Button>
            <Button onClick={analyze} disabled={!canAnalyze || analyzing} className="gradient-primary border-0 gap-2">
              <Search className="h-4 w-4" /> {analyzing ? "Analyzing..." : "Analyze Interactions"}
            </Button>
          </div>
            ...
            {result && (
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">{result.medicationCount} medications checked</span>
                </div>
                  ...
                  <div className="space-y-3">
                    {result.interactions.map((interaction, i) => (
                      <div key={i} className="rounded-xl border border-border bg-card shadow-card overflow-hidden">
                        <button className="w-full flex items-center justify-between p-4 text-left" onClick={() => setExpandedIdx(expandedIdx === i ? null : i)}>
                          <div className="flex items-center gap-3">
                            <AlertTriangle className={`h-5 w-5 ${interaction.severity === "High" ? "text-destructive" : interaction.severity === "Moderate" ? "text-warning" : "text-success"}`} />
